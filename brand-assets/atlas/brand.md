...
When Claude generates ATLAS-branded documents through the Apollo pipeline, these ten rules override any other inclination to embellish. They are non-negotiable.
...
2. **Emit the document title exactly once** — a single `<h1>` at the top with the template's canonical title (e.g., "FIELD SERVICE REPORT"). Do not repeat the title in subtitles, headers, or as decorative text. Do not translate the title.
...
4. **No spaced-out letter display** ("A T L A S"). The brand name appears exactly as written in this brand file or not at all.
...
8. **Signature blocks: emit a `<table>` with two cells. Each cell contains: role label (bold), typed name, blank line for handwritten signature, and a "Date: _______" line.** Do not emit the word "Signature" as a standalone line. For ATLAS field reports, the builder handles the technician attestation block — do not emit your own.
...
10. **Currency, dates, and numbers** follow standard US conventions: dates as "April 24, 2026", currency as "$1,234.56", percentages as "12.5%". Times use 24-hour for field reports ("14:27"), 12-hour with AM/PM elsewhere.

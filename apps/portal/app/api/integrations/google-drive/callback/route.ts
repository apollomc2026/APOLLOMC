...
export async function GET(request: Request) {
  ...
  let destination = new URL('/settings', process.env.NEXT_PUBLIC_APP_URL)
  ...
  try {
    ...
    const verified = verifyDriveOAuthState(state, auth.user.userId)
    destination = new URL(verified.returnTo, process.env.NEXT_PUBLIC_APP_URL)

...
export async function GET(request: Request) {
  ...
  const [turns, spec, evidence] = await Promise.all([
    db.from('apollo_conversation_turns').select('id, role, content, rationale, input_channel, transcription_confidence, critical_review_required, critical_review_confirmed, created_at').eq('conversation_id', id).order('sequence'),
  ...
  return NextResponse.json({ conversation_id: id, readiness: conversation.data.readiness, specification_version: conversation.data.current_spec_version, status: conversation.data.status, turns: turns.data.map(turn => ({ id: turn.id, role: turn.role, content: turn.content, reason: turn.rationale, createdAt: turn.created_at, inputChannel:turn.input_channel, transcriptionConfidence:turn.transcription_confidence, criticalReviewRequired:turn.critical_review_required, criticalReviewConfirmed:turn.critical_review_confirmed })), specification, job: latest ? { ...latest, artifact_url: latest.artifacts[0]?.web_view_url ?? null } : null, jobs: history })
}

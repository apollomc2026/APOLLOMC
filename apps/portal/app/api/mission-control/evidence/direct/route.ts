...
import { evidenceMagicMatches, extractEvidence, extractEvidenceFactsFromArtifact, MAX_EVIDENCE_BYTES, normalizeEvidenceMime, prepareEvidenceRetrieval, sanitizeEvidenceBytes } from '@/lib/mission-control/evidence'
...
export async function PATCH(request:Request){
  ...
  const originalHash=createHash('sha256').update(bytes).digest('hex');let status:'verified'|'failed'='verified';let retrievalKey=row.storage_key;let retrievalMime=row.mime_type;let retrievalHash=originalHash;let extractedText:string|undefined;let facts:Awaited<ReturnType<typeof extractEvidenceFactsFromArtifact>>=[]
  ...
  if(status==='verified'){try{facts=await extractEvidenceFactsFromArtifact({id,name:row.original_name,mime:row.mime_type,bytes,text:extractedText},prior.artifact.recommended_type)}catch{facts=[]};facts=facts.map(fact=>({...fact,last_editor:allowed.user.userId}))}

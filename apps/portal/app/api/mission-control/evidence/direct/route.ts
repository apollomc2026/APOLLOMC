...
import { evidenceMagicMatches, extractEvidence, extractEvidenceFactsWithTraceFromArtifact, type EvidenceExtractionTrace, MAX_EVIDENCE_BYTES, normalizeEvidenceMime, prepareEvidenceRetrieval, sanitizeEvidenceBytes } from '@/lib/mission-control/evidence'
...
export async function PATCH(request:Request){
  ...
  const originalHash=createHash('sha256').update(bytes).digest('hex');let status:'verified'|'failed'='verified';let retrievalKey=row.storage_key;let retrievalMime=row.mime_type;let retrievalHash=originalHash;let extractedText:string|undefined;let facts:Awaited<ReturnType<typeof extractEvidenceFactsWithTraceFromArtifact>>['facts']=[];let extractionTrace:EvidenceExtractionTrace|null=null
  ...
  if(status==='verified'){try{const extracted=await extractEvidenceFactsWithTraceFromArtifact({id,name:row.original_name,mime:row.mime_type,bytes,text:extractedText},prior.artifact.recommended_type);facts=extracted.facts;extractionTrace=extracted.trace}catch(error){facts=[];status='failed';extractionTrace=error&&typeof error==='object'&&'extractionTrace' in error?error.extractionTrace as EvidenceExtractionTrace:null};facts=facts.map(fact=>({...fact,last_editor:allowed.user.userId}))}
  const updated=await db.from('apollo_conversation_evidence').update({content_sha256:originalHash,retrieval_storage_key:retrievalKey,retrieval_mime_type:retrievalMime,retrieval_sha256:retrievalHash,size_bytes:bytes.length,extraction_status:status,extracted_facts:facts,extraction_trace:extractionTrace}).eq('id',id).eq('user_id',allowed.user.userId)

...
export async function GET() {
  ...
  const currentSpecifications = conversations.data?.length
    ...
    : new Map<string, CurrentSpecificationIdentity>()
  ...
  const missions = (conversations.data ?? []).map(mission => {
    ...
    const normalizedJobs = missionJobs.map(job => ({ id:job.id, deliverable_type:job.deliverable_type, state:job.state, progress_percent:job.progress_percent, message:job.status_message, artifacts:projectControlledArtifacts(job.id,((job.artifacts??[]) as ArtifactManifest[])), created_at:job.created_at, is_current_specification:jobMatchesCurrentSpecification({deliverable_type:job.deliverable_type,work_order:job.work_order as DocumentWorkOrder|null},identity) }))
    return { ...mission, title:identity?.title ?? mission.title, deliverable_type:identity?.deliverable_type ?? null, job:normalizedJobs.find(job=>job.is_current_specification) ?? null, jobs:normalizedJobs }
  })
  ...
  const active = allJobs.filter(job => job.is_current_specification&&!['delivered','failed','blocked','cancelled'].includes(job.state)).length + missions.filter(mission => mission.status !== 'archived' && !mission.job).length
...
async function serviceCurrentSpecifications(conversationIds:string[],userId:string) {
  ...
  for(const row of rows.data ?? []){
    ...
    if(title&&deliverableType)identities.set(String(row.conversation_id),{specification_id:String(row.id),specification_hash:String(row.content_hash),version:currentVersion,title,deliverable_type:deliverableType})
  }
  return identities
}

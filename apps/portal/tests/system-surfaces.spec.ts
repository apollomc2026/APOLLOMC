...
test('archive, telemetry, and settings are operational surfaces', async ({ page }) => {
  ...
  await expect(page.getByRole('link', { name:'Open Launch 01' })).toHaveAttribute('href', '#launch')
  await expect(page.getByRole('link', { name:'Open Reflight 01' })).toHaveAttribute('href', '#reflight')

...
test('mission control never hides a live quote behind archived copies or an arbitrary five-row limit',async({page})=>{
  const fixture=await (await page.request.get('/api/mission-control/overview')).json()
  const filler=Array.from({length:6},(_,index)=>({...fixture.missions[1],id:`filler-${index}`,title:`Operational mission ${index+1}`,status:'calibrating'}))
  await page.route('**/api/mission-control/overview',route=>route.fulfill({status:200,contentType:'application/json',json:{...fixture,missions:[{...fixture.missions[1],id:'dead-copy',title:'Archived Encore FSR',status:'archived'},...filler,{...fixture.missions[0],id:'quote-live',title:'US Foods · Quote / Estimate',status:'brief_ready'}]}}))
  await page.goto('/dashboard')
  await expect(page.getByRole('heading',{name:'US Foods · Quote / Estimate'}).first()).toBeVisible()
  await expect(page.getByRole('heading',{name:'Operational mission 6'})).toBeVisible()
  await expect(page.getByText('Archived Encore FSR')).toHaveCount(0)
})
...
test('archive, telemetry, and settings are operational surfaces', async ({ page }) => {
  ...
  await expect(page.getByText('Field Operations Proposal · Launch 01', { exact:true })).toBeHidden()
  await page.getByText('Deployment history', { exact:true }).click()
  await expect(page.getByText('Field Operations Proposal · Launch 01', { exact:true })).toBeVisible()
  await expect(page.getByText('Field Operations Proposal · Reflight 01', { exact:true })).toBeVisible()
...
test('delivered work opens a controlled review and accepts scoped revision directives', async ({ page }) => {
  ...
  await expect(page.getByText('Field Operations Proposal · DEPLOYMENT 02')).toBeVisible()
  await expect(page.getByText('Field Operations Proposal · DEPLOYMENT 01')).toBeVisible()
  ...
  const revisionRequest = page.waitForRequest(request => request.url().endsWith('/api/mission-control/revise') && request.method()==='POST')
  ...
  await revision
  expect((await revisionRequest).postDataJSON()).toMatchObject({job_id:'job-demo',request_id:expect.stringMatching(/^[0-9a-f-]{36}$/)})
...
test('the review workbench recovers a blocked run without labeling it as a draft', async ({ page }) => {
  ...
  await expect(page.getByText('Field Operations Proposal · EXECUTION 03')).toBeVisible({ timeout:15_000 })

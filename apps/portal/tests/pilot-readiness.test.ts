...
import { auditPilotRelease,PILOT_DELIVERABLES,type PilotAuditInput } from '../lib/mission-control/pilot-readiness'
...
describe('pilot release auditor',()=>{
  ...
  it('does not hide the newest failed mission behind an older delivered mission',()=>{
    const older=fixture('quote');older.conversation.created_at='2026-09-16T12:00:00Z';older.conversation.updated_at='2026-09-16T14:00:00Z'
    const newer=fixture('quote');newer.conversation.id='mission-quote-new';newer.conversation.created_at='2026-09-16T13:00:00Z';newer.conversation.updated_at='2026-09-16T13:00:00Z';newer.specification.id='spec-quote-new';newer.specification.conversation_id=newer.conversation.id
  ...
  it('rejects a regeneration that silently changes an approved field',()=>{
    const item=fixture('proposal')
    item.jobs[1].work_order.fields={pricing_detail:'Fixed fee: $18,750'}
    item.jobs[2].work_order.fields={...item.jobs[2].work_order.fields,pricing_detail:'Fixed fee: $99,999'}
    const report=auditPilotRelease({conversations:[item.conversation],specifications:[item.specification],evidence:[item.evidence],jobs:item.jobs,events:item.events})
    expect(report.classes.find(entry=>entry.deliverable_type==='proposal')!.gates.find(gate=>gate.key==='regeneration')).toMatchObject({passed:false})
  })

  it('does not let an earlier valid reflight hide a later corrupted reflight',()=>{
    const item=fixture('quote')
    const priorRevision=item.jobs[2]
    const laterId='40000000-0000-5000-a000-000000000003'
    const later={...priorRevision,id:laterId,created_at:'2026-09-16T12:20:00Z',completed_at:'2026-09-16T12:25:00Z',work_order:{...priorRevision.work_order,work_order_id:laterId,task_id:laterId,idempotency_key:`launch-${laterId}-${'a'.repeat(16)}`,fields:{...priorRevision.work_order.fields,revision_of:priorRevision.id,revision_instruction:'Alter the approved price without review.'}},artifacts:[{...priorRevision.artifacts[0],artifact_id:`artifact-${laterId}`,task_id:laterId,source_run_id:laterId,version:3}]}
    item.jobs.push(later)
    const report=auditPilotRelease({conversations:[item.conversation],specifications:[item.specification],evidence:[item.evidence],jobs:item.jobs,events:item.events})
    expect(report.classes.find(entry=>entry.deliverable_type==='quote')!.gates.find(gate=>gate.key==='regeneration')).toMatchObject({passed:false})
  })
  ...
  it('rejects a PDF whose final specialist-verification proof is missing',()=>{
    const item=fixture('quote')
    delete (item.jobs[2].artifacts[0].integrity as {factual_content_sha256?:string}).factual_content_sha256
    const report=auditPilotRelease({conversations:[item.conversation],specifications:[item.specification],evidence:[item.evidence],jobs:item.jobs,events:item.events})
    const artifact=report.classes.find(entry=>entry.deliverable_type==='quote')!.gates.find(gate=>gate.key==='artifact')!
    expect(artifact).toMatchObject({passed:false})
    expect(artifact.evidence).toContain('final factual verification=missing')
  })

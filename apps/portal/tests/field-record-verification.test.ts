...
describe('deterministic field-record verification', () => {
  ...
  it('accepts a spelled-out state while preserving the approved street, city, and postal code',()=>{
    const order={deliverable_type:'fsr',fields:{work_order_number:'WO-1',site_name:'Northstar',site_address:'100 Industrial Way, Worcester, MA 01608',customer_contact_onsite:'Avery Morgan',visit_date:'2026-09-17',arrival_time:'07:00',departure_time:'16:30',technician_name:'Morgan Reed',equipment_asset_id:'ASSET-1',equipment_make_model:'Model X',issue_reported:'Alarm',warranty_status:'not applicable',time_on_site_hours:'9.5',follow_up_required:'none',work_performed:'07:00 | Inspected equipment'}} as unknown as DocumentWorkOrder
    const html='<p>WO-1 Northstar 100 Industrial Way, Worcester, Massachusetts 01608 Avery Morgan September 17, 2026 7:00 AM 4:30 PM Morgan Reed ASSET-1 Model X Alarm not applicable 9.5 none</p><p>07:00 Inspected equipment</p>'
    expect(verifyFieldRecord(order,html).required).toBe(true)
    expect(()=>verifyFieldRecord(order,html.replace('100 Industrial Way','200 Industrial Way'))).toThrow(/site_address/)
  })

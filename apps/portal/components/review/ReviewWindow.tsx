...
export default function ReviewWindow({ missionId }:{ missionId:string }) {
  ...
  const display = useMemo(() => specification?missionDisplayIdentity(specification):null,[specification])
  ...
  async function revise(instructionOverride?:string) {
    ...
    try {
      ...
      const response = await fetch('/api/mission-control/revise',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({job_id:latest.id,instruction:scoped,request_id:crypto.randomUUID()})})
      const body = await response.json()
  ...
  async function retry() {
    if (!latest || !['blocked','failed'].includes(latest.state) || working) return
    setWorking(true); setError('')
    try {
  ...
  return <main className="review-workbench">
    ...
    <section className="review-history"><header><div><span>03 / VERSION LINEAGE</span><h2>Nothing overwritten.</h2></div><p>Every revision remains attributable to its source job and instruction.</p></header>{record.jobs.length?<div>{record.jobs.map((job,index)=><article key={job.id}><div className="review-version-icon">{job.state==='delivered'?<CheckCircle2/>:<FileClock/>}</div><div><span>{display?.displayTitle??specification.mission.title} · {job.artifacts[0]?.version ? `DEPLOYMENT ${String(job.artifacts[0].version).padStart(2,'0')}` : `EXECUTION ${String(record.jobs.length-index).padStart(2,'0')}`}</span><strong>{job.revision_instruction || 'Approved specification execution'}</strong><small>{new Date(job.created_at).toLocaleString()} · {job.id.slice(0,8).toUpperCase()}</small></div><em>{job.state.replace(/-/g,' ')}</em>{job.artifacts[0]?.web_view_url?<a href={job.artifacts[0].web_view_url} target="_blank" rel="noreferrer"><ExternalLink/> Open</a>:null}</article>)}</div>:<div className="review-no-history">No execution versions exist yet.</div>}</section>
  </main>
}

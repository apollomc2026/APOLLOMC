...
export function MissionControl() {
  ...
  function resetMission() {
    setTurns([opening]); setSpecification(null); setReadiness(0); setConversationId(null); setSpecificationVersion(0); setJobId(null); setJobState(null); setArtifactUrl(null); setDraft(''); setAcceptUnresolved(false); setDecisionAnswers({}); setError(null); window.localStorage.removeItem(STORAGE_KEY); window.history.replaceState(window.history.state, '', '/dashboard')
  }

  function submitDecisionAnswers(useRecommendations: boolean) {
    const answered = questions.flatMap((question, index) => decisionAnswers[question]?.trim() ? [`${index + 1}. ${question}\nAnswer: ${decisionAnswers[question].trim()}`] : [])
    const remaining = questions.filter(question => !decisionAnswers[question]?.trim())
    ...
    void submit([answered.length ? `My answers to the mission checkpoint:\n\n${answered.join('\n\n')}` : '', recommendationDirective].filter(Boolean).join('\n\n'))
  }
  ...
  return <div className="mc-workspace">
    ...
    <div className="mc-grid">
      <section className="mc-console" aria-label="Mission conversation">
        ...
        <div className={`mc-composer${questions.length ? ' response-pending' : ''}`}>{questions.length ? <section className="mc-question-batch"><header><div><span>Mission checkpoint</span><strong>Please answer the following together</strong></div><b>{questions.length} open</b></header><ol>{questions.map(question => <li key={question}>{question}</li>)}</ol><button type="button" onClick={() => void submit(`Use your expert recommendations for every unresolved decision that can be responsibly inferred from the mission. Clearly mark recommendations as assumptions. Do not invent names, credentials, evidence, prices, dates, legal terms, or client-specific facts. Leave only decisions that genuinely require my input unresolved.\n\nOpen decisions:\n${questions.map((question, index) => `${index + 1}. ${question}`).join('\n')}`)} disabled={working}>Use expert recommendations</button></section> : null}<div className="mc-prompt-label">{questions.length ? 'One response can answer every open item' : 'Respond naturally—one answer can resolve several facts.'}</div><textarea value={draft} onChange={event => setDraft(event.target.value)} onKeyDown={event => { if (event.key === 'Enter' && !event.shiftKey) { event.preventDefault(); void submit() } }} placeholder={questions.length ? 'Answer the questions above in one response, or use expert recommendations…' : 'Describe what must be accomplished, who it is for, and what you already have…'} rows={4}/><div className="mc-composer-tools"><div className="mc-input-tools"><input ref={fileRef} type="file" multiple hidden accept=".pdf,.docx,.xlsx,.csv,.txt,.png,.jpg,.jpeg" onChange={event => void attachEvidence(event.target.files)}/><button type="button" className="mc-icon-button" onClick={() => fileRef.current?.click()} disabled={working}><Paperclip size={18}/><span>Add evidence</span></button><VoiceControl disabled={working} onTranscript={acceptVoiceTranscript}/></div><button type="button" className="mc-send" onClick={() => void submit()} disabled={working}><span>{working ? 'Interpreting' : 'Answer all and continue'}</span><ArrowUp size={18}/></button></div>{error ? <p className="mc-error">{error}</p> : null}</div>
      </section>

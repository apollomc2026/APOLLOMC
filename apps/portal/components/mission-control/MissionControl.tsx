...
export function MissionControl() {
  ...
  async function approveBrief() {
    if (!specification || readiness < 75 || (questions.length > 0 && !acceptUnresolved)) return
    setWorking(true); setError(null)
    try {
      if (conversationId) {
        const response = await fetch('/api/mission-control/approve', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ conversation_id: conversationId, version: specificationVersion, unresolved_items_accepted: acceptUnresolved ? questions : [] }) })
      ...
      setSpecification({ ...specification, approval: { ...specification.approval, status: 'approved', approved_by: 'current-user', approved_at: new Date().toISOString(), unresolved_items_accepted: acceptUnresolved ? questions : [] } })

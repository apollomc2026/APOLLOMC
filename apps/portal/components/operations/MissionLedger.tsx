...
export function MissionLedger({ view }:{ view:'archive'|'telemetry'|'dashboard' }) {
  ...
  const commandMission = useMemo(() => missions.find(m => m.job && !['delivered','failed','blocked','cancelled'].includes(m.job.state)) ?? missions.find(m => m.job?.state === 'delivered') ?? missions[0] ?? null,[missions])
  ...
  if (view === 'dashboard') return <div className="ops-stack">
    {commandMission ? <section className="dashboard-command-deck">
      ...
      <div className="dashboard-command-copy"><span>CURRENT COMMAND · SPECIFICATION V{commandMission.current_spec_version}</span><h2>{commandMission.title}</h2><p>{commandMission.job?.state === 'delivered' ? 'A controlled draft is ready. Launch a clean reflight here, or enter Telemetry for directed revision and complete version lineage.' : commandMission.job ? `${commandMission.job.message} · ${commandMission.job.progress_percent}%` : `Mission calibration is ${commandMission.readiness}% complete.`}</p><div className="dashboard-command-status"><i/><strong>{commandMission.job?.state?.replace(/-/g,' ') ?? `${commandMission.readiness}% ready`}</strong><small>Updated {new Date(commandMission.updated_at).toLocaleString()}</small></div></div>
      <div className="dashboard-command-actions">{commandMission.job?.state === 'delivered' ? <button type="button" onClick={()=>void regenerate(commandMission)} disabled={launchingId!==null}><Rocket/><strong aria-live="assertive">{launchingId===commandMission.id && countdown!==null ? countdown : 'REGENERATE DELIVERABLE'}</strong><small>Approved facts and evidence remain locked</small></button> : <Link className="dashboard-primary-action" href={`/new-mission?mission=${commandMission.id}`}><Rocket/><strong>RESUME MISSION</strong><small>Return to calibration and launch control</small></Link>}<div><Link href={`/telemetry?mission=${commandMission.id}`}><Gauge/>Telemetry</Link><Link href={`/new-mission?mission=${commandMission.id}&edit=1`}><PencilLine/>Edit mission data</Link></div></div>

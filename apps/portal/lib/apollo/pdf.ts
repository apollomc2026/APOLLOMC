...
export async function buildPdf(args: BuildPdfArgs): Promise<Buffer> {
  ...
  try {
    ...
    const disclaimerHtml = args.disclaimer
      ...
      : ''
    // Ledger packets carry only the selected entity and reporting period. Product
    // attribution is intentionally omitted so APOLLO or On Spot identity can
    // never leak into a client-branded or white-label artifact.
    const genre = genreForSlug(args.template.slug)

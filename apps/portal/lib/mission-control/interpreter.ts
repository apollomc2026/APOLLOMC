...
export function explicitMissionArtifact(text: string): SupportedMissionArtifact | null {
  ...
  if (/\b(?:contract|agreement|warranty)\s+(?:review|analysis|intelligence)|\breview\s+(?:this|the|my|an?)\s+(?:contract|agreement|warranty)\b/i.test(text)) return 'contract-intelligence-review'
...
export function recommendMissionArtifact(text: string) {
  ...
  if (explicit === 'contract-intelligence-review') return { family: 'Contract intelligence', type: 'contract-intelligence-review', playbook: 'contract-intelligence', rationale: 'The operator needs an existing agreement translated into clause-linked status, obligations, dates, entitlements, risks, and practical actions.', sections: ['Contract command summary', 'Status and critical dates dashboard', 'Plain-English contract map', 'Obligations, rights, and dependencies', 'Money, coverage, and value capture', 'Commonly missed clauses', 'Contract leverage playbook', 'Risk and professional escalation', 'Action calendar and checklist', 'Clause and source index'], checks: ['complete-document-coverage', 'clause-source-traceability', 'date-status-consistency', 'no-unsupported-legal-conclusions'] }
  ...
  if (explicit === 'final-qc-report') return { family: 'Quality control closeout', type: 'final-qc-report', playbook: 'quality-control-closeout', rationale: 'The requested outcome is a consolidated, evidence-led acceptance record for completed work.', sections: ['Project and inspection summary', 'Scope and acceptance criteria', 'Test instruments and methods', 'Per-item results', 'Exceptions and corrective actions', 'Certification and acceptance'], checks: ['acceptance-criteria', 'test-result-traceability', 'evidence-custody', 'unverified-claims'] }
  ...
  if (explicit === 'quote') return { family: 'Commercial quote', type: 'quote', playbook: 'commercial-quote', rationale: 'The mission requires an evidence-grounded, commercially controlled offer with approved scope, quantities, pricing, validity, and payment terms.', sections: ['Customer and project', 'Scope of work', 'Line items and investment', 'Schedule and validity', 'Terms, assumptions, and exclusions', 'Acceptance'], checks: ['scope-completeness', 'commercial-figure-preservation', 'market-pricing-provenance', 'human-pricing-approval'] }
...
function extractFacts(text: string, now: Date): MissionFact[] {
  ...
  const audience = text.match(/Primary audience:\s*(.+?)(?=\.\s+(?:Deadline|Required format|Presentation calibration):|$)/i)
    ?? text.match(/(?:send|give|present|submit)(?:\s+it)?\s+to\s+(?:the\s+)?([^,.]+)/i)

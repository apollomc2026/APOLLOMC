...
export function auditPilotRelease(input:PilotAuditInput):{passed:boolean;passed_classes:number;total_classes:number;classes:PilotClassAudit[]} {
  ...
  const classes=PILOT_DELIVERABLES.map(deliverableType=>{
    ...
    const unlabeledInferences=spec.content.facts.filter(fact=>fact.source==='inferred'&&!spec.content.assumptions.some(item=>item.startsWith(`${fact.label}:`)&&item.includes('expert recommendation')))
    ...
    const launchKeys=jobs.map(job=>job.work_order.idempotency_key)
    ...
    const gates:PilotGate[]=[
      ...
      {key:'calibration',label:'Minimal-friction calibrated specification',passed:spec.content.open_questions.length===0&&unresolvedConflicts.length===0&&invalidSupersessions.length===0&&unsafeInferredExecution.length===0&&unlabeledInferences.length===0,evidence:`${spec.content.open_questions.length} open; ${unresolvedConflicts.length} unresolved conflicts; ${invalidSupersessions.length} unauditable supersession decision(s); ${unsafeInferredExecution.length} unauthorized inference(s) entered execution; ${unlabeledInferences.length} unlabeled inference(s); ${spec.content.facts.filter(fact=>fact.source==='inferred').length-unlabeledInferences.length} labeled assumption(s) retained.`},
      ...
      {key:'artifact',label:'Controlled branded PDF artifact',passed:Boolean(latestOrder)&&artifacts.length>0&&artifacts.every(artifact=>artifactIsControlled(artifact,latestOrder!)),evidence:`${artifacts.length} artifact(s); brand=${latestOrder?.brand_id??'missing'}; style=${latestOrder?.style_id??'missing'}; parsed PDF integrity=${artifacts.every(artifact=>Boolean(artifact.integrity))?'recorded':'missing'}; final factual verification=${artifacts.every(artifact=>artifact.integrity?.verification_profile==='specialist-pdf-v1'&&/^[a-f0-9]{64}$/.test(artifact.integrity?.factual_content_sha256??''))?'recorded':'missing'}; output authority=${latestOrder&&artifacts.every(artifact=>artifact.specification_hash===latestOrder.trace?.specification_hash)?'bound':'mismatched'}.`},

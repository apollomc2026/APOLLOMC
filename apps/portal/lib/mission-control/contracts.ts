...
export interface MissionFact { key: string; label: string; value: string; source: 'user' | 'inferred' | 'default'; confidence: number }
export interface DeliverableSpecification {
  ...
  sources: Array<{ id: string; name: string; status: 'pending' | 'verified' | 'conflict' }>
